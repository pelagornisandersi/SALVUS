...
def send_file_worker(room_code):

    if not selected_file:
        update_status("Select a file first")
        return

    

    if not room_code:
        update_status("Enter room code")
        return
    ...
    try:
        ...
        update_status("Waiting for Receiver...")
        ...
        update_status("Transfer Complete!")
...
def receive_file_worker(room_code):
    ...
    try:
        ...
        if not paired_ip:
            ...
            update_status("Scan QR code first")
        ...
        update_status(f"Received {filename}")
    except Exception as e:
